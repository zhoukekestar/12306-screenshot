...
export default function TicketCard({ initialInfo, onReset }: TicketCardProps) {
  ...
  return (
    <div className="w-full max-w-md flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      >
        {/* 顶部：站点信息 */}
        <div className="flex justify-between items-center mb-8 border-b-2 border-dashed border-gray-200 pb-8">
          <div className="text-center w-2/5">
            {isEditing ? (
              <input 
                className="w-full text-center text-3xl font-black text-gray-900 border-b-2 border-blue-400 focus:outline-none bg-blue-50 rounded px-1" 
                value={info.departureStation} 
                onChange={e => handleChange(e, 'departureStation')}
                placeholder="出发站"
              />
            ) : (
              <div className="text-4xl sm:text-5xl font-black text-gray-900 truncate">
          ...
          <div className="flex flex-col items-center justify-center w-1/5">
            <div className="text-gray-400 font-bold mb-1 text-sm tracking-widest">
              {isEditing ? (
                <input 
                  className="w-full text-center text-lg font-bold text-blue-600 border-b-2 border-blue-400 focus:outline-none bg-blue-50 rounded" 
        ...
        <div className="flex flex-col gap-6">
          <div className="flex justify-between items-end bg-gray-50 p-4 rounded-2xl">
            <span className="text-xl text-gray-500 font-medium">发车时间</span>
            <div className="flex flex-col items-end">
              {isEditing ? (
                <>
                   <input className="text-right text-lg text-gray-500 border-b border-blue-300 focus:outline-none mb-1 bg-transparent" value={info.date} onChange={e => handleChange(e, 'date')} placeholder="日期"/>
                   <input className="text-right text-4xl font-black text-gray-900 border-b border-blue-300 focus:outline-none bg-transparent" value={info.time} onChange={e => handleChange(e, 'time')} placeholder="时间"/>
                </>
              ) : (
                <>
                  <span className="text-xl text-gray-600 font-bold mb-1">{info.date || '未知日期'}</span>
                  <span className="text-5xl font-black text-red-600 tracking-tight">{info.time || '--:--'}</span>
                </>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl">
            <span className="text-xl text-gray-500 font-medium">座位号</span>
            {isEditing ? (
              <input 
                className="text-right text-4xl font-black text-gray-900 border-b border-blue-300 focus:outline-none bg-transparent w-1/2" 
                value={info.seat} 
                onChange={e => handleChange(e, 'seat')}
                placeholder="座位号"
              />
            ) : (
              <span className="text-4xl font-black text-gray-900">{info.seat || '无座'}</span>
            )}
          </div>
        </div>
